  ...
  function request() {
    ...
    $.ajax({
      ...
      beforeSend: function (xhr) {
        if (auth) {
          xhr.setRequestHeader ("Authorization", "Basic " + btoa("depviz:" + auth));
        }
      },
      ...
      error: function(xhr, status, error) {
        if (xhr.status == 401) {
          var input = window.prompt("auth ?")
          $('input[name="auth"]').val(input) // FIXME: find something more secure, i.e. cookies
          auth = input
          request()
          return
        }
